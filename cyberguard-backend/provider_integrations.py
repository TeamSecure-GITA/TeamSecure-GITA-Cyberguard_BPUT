...
def provider_readiness() -> dict[str, Any]:
    required = {
        "response_webhook": ["CYBERGUARD_RESPONSE_WEBHOOK_URL"],
        "alert_webhook": ["CYBERGUARD_ALERT_WEBHOOK_URL"],
        "siem": ["CYBERGUARD_SIEM_URL"],
        "urlhaus": ["CYBERGUARD_URLHAUS_AUTH_KEY"],
        "abuseipdb": ["CYBERGUARD_ABUSEIPDB_KEY"],
        "tenant_immunity": ["CYBERGUARD_TENANT_IMMUNITY_URL", "CYBERGUARD_TENANT_IMMUNITY_SECRET"],
    }
    checks = {name: {"configured": all(os.getenv(key, "").strip() for key in keys), "missing": [key for key in keys if not os.getenv(key, "").strip()]} for name, keys in required.items()}
    return {"ready": all(item["configured"] for item in checks.values()), "checks": checks, "note": "Credentials are intentionally read from the environment or secret manager and are never generated by the application."}
