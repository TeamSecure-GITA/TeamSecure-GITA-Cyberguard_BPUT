...
export default function ThreatIntelligence({ accessToken, incidents }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get(`${apiBaseUrl}/api/v1/incidents/${id}/genome`, config),
  ...
  const runSimulation = async () => {
    ...
    setBusy(true);
    try {
      const response = await axios.post(`${apiBaseUrl}/api/v1/incidents/${selectedIncident.database_id}/simulate`, { actions: selectedActions }, config);
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
      <Panel icon={BrainCircuit} eyebrow="01 / threat DNA" title="Genome fingerprint">
        <div className="genome-mark"><div className="genome-core">{genome?.fingerprint?.slice(0, 4) || 'DNA'}</div><div className="genome-orbit genome-orbit-one" /><div className="genome-orbit genome-orbit-two" /></div>
        <div className="grid grid-cols-2 gap-3 mt-3">{Object.entries(genome?.vectors || {}).map(([key, value]) => <div className="intel-data-cell" key={key}><span>{key.replace('_', ' ')}</span><strong>{Array.isArray(value) ? value.join(', ') || 'none' : value}</strong></div>)}</div>
      </Panel>
      <Panel icon={Link2} eyebrow="02 / shadow campaign" title="Cross-incident correlation">
        <div className="campaign-banner"><span>{correlations?.campaign_id || 'Awaiting correlation'}</span><strong>{correlations?.stage || 'Dormant'}</strong></div>
        <div className="intel-list">{correlations?.related_incidents?.length ? correlations.related_incidents.map((item) => <div className="intel-row" key={item.incident_id}><span>{item.incident_id}</span><b>{item.score}%</b><small>{item.reason}</small></div>) : <p className="intel-empty">No matching campaign evidence in the current window.</p>}</div>
      </Panel>
      <Panel icon={GitBranch} eyebrow="03 / cyber time machine" title="Attack chain timeline">
        <div className="chain-timeline">{(timeline || []).map((event) => <div className={`chain-event ${event.status}`} key={event.id}><span>{event.id}</span><div><strong>{event.label}</strong><small>{event.detail}</small></div><time>{new Date(event.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time></div>)}</div>
      </Panel>
      <Panel icon={Activity} eyebrow="04 / future threat predictor" title="Risk forecast">
        <div className="forecast-chart"><ResponsiveContainer width="100%" height={190}><LineChart data={forecast?.forecast || []}><XAxis dataKey="label" stroke="#688297" fontSize={10} /><YAxis domain={[0, 100]} stroke="#688297" fontSize={10} /><Tooltip contentStyle={{ background: '#0d1c2d', border: '1px solid #315267', color: '#e8f1f7' }} /><Line type="monotone" dataKey="risk" stroke="#ff7d86" strokeWidth={3} dot={{ fill: '#ff7d86', r: 4 }} /></LineChart></ResponsiveContainer></div>
        <div className="flex flex-wrap gap-2">{(forecast?.drivers || []).map((driver) => <span className="evidence-chip" key={driver}>{driver}</span>)}</div>
      </Panel>
      ...
      <Panel icon={Swords} eyebrow="08 / AI defender vs attacker" title="Battle arena" action={<button className="icon-action" title="Run battle simulation" onClick={runSimulation} disabled={busy}><Play size={14} /></button>}>
      ...
      <Panel icon={HeartPulse} eyebrow="09 / what-if simulator" title="Projected response impact">
        {simulation ? <><div className="simulation-score"><div><span>Current</span><strong>{simulation.current_risk}%</strong></div><div className="simulation-arrow">→</div><div><span>Projected</span><strong className="text-emerald-300">{simulation.projected_risk}%</strong></div></div><RiskBar value={simulation.projected_risk} /></> : <button className="wide-action" onClick={runSimulation}><Play size={14} /> Simulate selected controls</button>}
      </Panel>
    </div>
