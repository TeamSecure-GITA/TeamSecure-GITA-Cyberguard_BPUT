...
export default function NotificationsPanel({ accessToken, workload = null }) {
  ...
  useEffect(() => {
    ...
    axios.post(`${apiBaseUrl}/api/v1/alert-routing`, { incidents: workload?.incidents || [], analysts: workload?.analysts || [] }, { headers: { Authorization: `Bearer ${accessToken}` } }).then((response) => setRouting(response.data)).catch(() => setRouting(null));
  }, [accessToken, workload]);
  ...
  return <section className="bg-cardBg border border-slate-700/60 rounded-xl p-6 shadow-lg">
    ...
    {routing && <div className="mb-4 rounded-lg border border-cyan-500/25 bg-cyan-500/5 p-3 text-[10px] text-cyan-200">
      <div className="font-bold uppercase mb-1">Fatigue-aware routing</div>
      <div>{routing.route_summary.high_priority} high-priority alerts · avg load {routing.route_summary.avg_load} · analysts {routing.route_summary.available_analysts}</div>
    </div>}
